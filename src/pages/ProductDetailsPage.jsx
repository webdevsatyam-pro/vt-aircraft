...
import starterKitComponents from "../assets/images/starter_kit_user_contents.jpg";
import viggenStarterKit from "../assets/images/viggen_starter_kit.jpg";
import mustangStarterKit from "../assets/images/mustang_starter_kit.jpg";
...
const ImagePlaceholder = ({ label, src, onClick, aspect = "aspect-[4/3]" }) => {
  ...
  if (error || !src) {
    return (
      <div
        className={`flex flex-col items-center justify-center p-6 bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl ${aspect} text-center group hover:border-[#dd3333] transition-colors duration-250`}>
        <svg
          className="w-8 h-8 text-gray-300 group-hover:text-[#dd3333] mb-2 transition-colors"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24">
          <path
        ...
        <span className="text-[11px] text-gray-700 font-extrabold mt-1 uppercase tracking-wide px-2">
          {label}
        </span>
      </div>
    );
  ...
  return (
    <div
      ...
      <div
        className={`${aspect} overflow-hidden bg-gray-50 flex items-center justify-center p-2`}>
        <img
          src={src}
          alt={label}
...
export default function ProductDetailsPage() {
  ...
  const product =
    allCatalog.find((p) => p.slug === id || p.id === id) || productsData[0];

  const [selectedImage, setSelectedImage] = useState(
    Array.isArray(product.images)
      ? product.images[0]
      : product.image || "/src/assets/images/vt_trainer_hero_1784882888882.jpg",
  );
  ...
  const isViggen =
    product.id === "vt-viggen" ||
    product.id === "vt-simple-trainer-pnp" ||
    product.name.toLowerCase().includes("viggen") ||
    product.name.toLowerCase().includes("vission");
  const isMustang =
    product.id === "vt-lipo-battery-3s" ||
    product.name.toLowerCase().includes("mustang");
  const isSpitfire =
    product.id === "vt-6x-transmitter" ||
    product.name.toLowerCase().includes("spitfire");
  const isGuineaPig =
    product.id === "vt-guinea-pig" ||
    product.name.toLowerCase().includes("guinea");
  const isStorch =
    product.id === "vt-storch" || product.name.toLowerCase().includes("storch");
  const isExplorer =
    product.id === "vt-explorer" ||
    product.name.toLowerCase().includes("explorer");
  const isSpear =
    product.id === "vt-spear" || product.name.toLowerCase().includes("spear");
  const isRaptor =
    product.id === "vt-raptor" || product.name.toLowerCase().includes("raptor");
  const isOldFogey =
    product.id === "vt-old-fogey" ||
    product.name.toLowerCase().includes("fogey");
  const isBaron =
  ...
  const currentBaseOriginalPriceStr = formatINR(
    baseOriginalPriceVal * quantity,
  );
  ...
  const relatedProducts = allCatalog.filter(
    (p) =>
      p.id !== product.id &&
      p.id !== "vt-trainer-spare-wing" &&
      p.id !== "vt-flight-sim-dongle",
  );
  const productReviews = reviewsData.filter(
    (r) =>
      r.productId === product.id || product.id.includes("vt-simple-trainer"),
  );
    ...
    if (product) {
      setSelectedImage(
        Array.isArray(product.images)
          ? product.images[0]
          : product.image ||
              "/src/assets/images/vt_trainer_hero_1784882888882.jpg",
      );
      setKitOption(null);
  ...
  React.useEffect(() => {
    ...
    const startAutoScroll = () => {
      ...
      ); // Autoscroll every 2 seconds on mobile, 1 second on desktop
    };
  ...
  const shopifyUrl =
    product.shopifyUrl ||
    `https://your-shopify-store.myshopify.com/products/${product.slug || product.id}`;
  const showDetailedSections =
    product.id !== "vt-f16" && product.id !== "vt-sbach342";
        ...
        <div className="lg:col-span-5 space-y-6 flex flex-col justify-between">
          <div className="space-y-4">
                ...
                <>
                  ...
                  {product.originalPrice && (
                    <span className="text-sm text-gray-400 line-through font-normal">
                      ₹
                      {Math.round(
                        (product.originalPrice || 0) * quantity * 80,
                      ).toLocaleString("en-IN")}
                      .00
                    </span>
                  )}
            ...
            {isAircraft && kitOption === "starter" && (
              <div className="pt-4 space-y-3 border-t border-gray-100">
                <p className="font-bold text-[#e03a3a] text-xs sm:text-sm">
                  Starter Kit Contents (Everything you need to fly):
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                  <ul className="list-disc pl-5 space-y-1 text-xs text-gray-700 font-medium">
                    <li>
                      High-Strength EPP Airframe Kit (Reinforced with Carbon
                      Fiber)
                    </li>
                    <li>
                      {isGuineaPig
                        ? "2 Nos. DYS 2826 1400kv Brushless Motors"
                        : "DYS 2826 1400kv Brushless Motor"}
                    </li>
                    <li>
                      {isGuineaPig
                        ? "2 Nos. 30A Electronic Speed Controllers (ESCs)"
                        : "30A Electronic Speed Controller (ESC)"}
                    </li>
                    <li>
                      {isViggen || isGuineaPig
                        ? "4 Nos. TowerPro 9g Metal Gear Servos"
                        : isMustang || isSpitfire || isStorch
                          ? "5 Nos. TowerPro 9g Metal Gear Servos"
                          : "3 Nos. TowerPro 9g Metal Gear Servos"}
                    </li>
                    ...
                    <li>
                      Complete hardware pack (pushrods, landing gear, wheels,
            ...
            {isAircraft &&
              ...
              electronicsOption === "with-electronics" && (
                <div className="pt-4 space-y-3 border-t border-gray-100">
                  <p className="font-bold text-[#e03a3a] text-xs sm:text-sm">
                    Optional Electronics Pack Contents (Included with "With
                    Electronics" option):
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                    <ul className="list-disc pl-5 space-y-1 text-xs text-gray-700 font-medium">
                      ...
                      <li>
                        0.5 Meter Red (32 Strands/0.2mm Tinned copper ) Silicone
                        Wire.
                      </li>
                      <li>
                        0.5 Meter Black (32 Strands/0.2mm Tinned copper )
                        Silicone Wire
                      </li>
                    ...
                    <div className="rounded-xl overflow-hidden border border-gray-200 shadow-xs bg-gray-50 aspect-video flex items-center justify-center p-1">
                      <img
                        src="/src/assets/images/epp_trainer_electronics.jpg?v=2"
                ...
                kitOption ? (
                  kitOption === "starter" ? (
                    ...
                    <div className="text-xs text-gray-400 italic">
                      Please select an electronics option to view the final
                      price.
                    </div>
                  )
                ) : (
                  <div className="text-xs text-gray-400 italic">
                    Please select a Kit Option to view the final price.
                  </div>
                )
              ) : (
                <div className="flex items-baseline gap-2">
          ...
          <div className="space-y-6 pt-4 border-t border-gray-200">
                  ...
                  onClick={(e) => {
                    if (isAircraft) {
                      if (!kitOption) {
                        e.preventDefault();
                        alert(
                          "Please select a Kit Option (Starter Kit or Airframe Kit) to proceed.",
                        );
                      } else if (
                        kitOption === "airframe" &&
                        !electronicsOption
                        ...
                        e.preventDefault();
                        alert(
                          "Please select an Electronics option (No Electronics or With Electronics) to proceed.",
                        );
                      }
                    }
                  }}
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-gray-100">
              ...
              <a
                href="https://www.instagram.com/rcplanehub/"
                target="_blank"
                rel="noopener noreferrer"
                className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:border-gray-900 hover:text-gray-900 transition">
            ...
            <div className="p-6 sm:p-8 text-xs sm:text-sm text-gray-700 leading-relaxed">
              {activeTab === "description" && (
                <div className="space-y-10">
                  {/* Description Header */}
                  <div className="space-y-4">
                    <h3 className="text-2xl font-bold text-[#dd3333]">
                      {displayName}
                    </h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="bg-gray-50 p-6 rounded-2xl border border-gray-150 space-y-4">
                      <h4 className="text-center font-bold text-[#dd3333] text-base uppercase tracking-wider border-b pb-2">
                        Specifications
                      </h4>
                        ...
                        {isViggen ? (
                          <>
                            ...
                            <li className="flex justify-between border-b pb-1">
                              ...
                              <span className="font-bold text-gray-900">
                                5.25in/133mm from Leading edge of rudder
                              </span>
                            </li>
                            <li className="flex justify-between pb-1">
                              <span>Control Throws:</span>
                              <span className="font-bold text-gray-900">
                                Elevons: 90 – 120 EXPO: 400
                              </span>
                            </li>
                          </>
                        ) : isMustang ? (
                          <>
                            ...
                            <li className="flex justify-between border-b pb-1">
                              <span>CG Location:</span>
                              <span className="font-bold text-gray-900">
                                2.75in/70mm from the leading edge of the wing
                              </span>
                            </li>
                            <li className="flex justify-between pb-1">
                              <span>Control Throws:</span>
                              <span className="font-bold text-gray-900">
                                Elevons: 160 EXPO: 300
                              </span>
                            </li>
                          </>
                        ) : isSpitfire ? (
                          <>
                            ...
                            <li className="flex justify-between border-b pb-1">
                              <span>CG Location:</span>
                              <span className="font-bold text-gray-900">
                                2.75in/68mm from the leading edge of the wing
                              </span>
                            </li>
                            <li className="flex justify-between pb-1">
                              <span>Control Throws:</span>
                              <span className="font-bold text-gray-900">
                                Elevons: 100 EXPO: 300
                              </span>
                            </li>
                          </>
                        ) : isGuineaPig ? (
                          <>
                            ...
                            <li className="flex justify-between border-b pb-1">
                              <span>CG Location:</span>
                              <span className="font-bold text-gray-900">
                                50mm from the leading edge of the wing
                              </span>
                            </li>
                            <li className="flex justify-between pb-1">
                              <span>Control Throws:</span>
                          ...
                          <>
                            ...
                            <li className="flex justify-between border-b pb-1">
                              <span>CG Location:</span>
                              <span className="font-bold text-gray-900">
                                2in/50mm from the leading edge of the wing
                              </span>
                            </li>
                            <li className="flex justify-between pb-1">
                              <span>Control Throws:</span>
                          ...
                          <>
                            ...
                            <li className="flex justify-between border-b pb-1">
                              <span>CG Location:</span>
                              <span className="font-bold text-gray-900">
                                2.25in/57mm from leading edge of the wing
                              </span>
                            </li>
                            <li className="flex justify-between border-b pb-1">
                            ...
                            <li className="flex justify-between pb-1">
                              <span>Control Throws:</span>
                              <span className="font-bold text-gray-900">
                                Aileron: 12°, Elevator: 12°, Rudder: 12°
                              </span>
                            </li>
                          </>
                        ) : isSpear ? (
                          ...
                          <>
                            ...
                            <li className="flex justify-between border-b pb-1">
                              <span>CG Location:</span>
                              <span className="font-bold text-gray-900">
                                76-89mm from the leading edge of the wing
                              </span>
                            </li>
                            <li className="flex justify-between pb-1">
                              <span>Control Throws:</span>
                              <span className="font-bold text-gray-900">
                                Elevons: 120 EXPO: 300
                              </span>
                            </li>
                          </>
                        ) : isBaron ? (
                          <>
                            ...
                            <li className="flex justify-between border-b pb-1">
                              <span>CG Location:</span>
                              <span className="font-bold text-gray-900">
                                2.3in/58.7mm from leading edge of wing
                              </span>
                            </li>
                            <li className="flex justify-between border-b pb-1">
                              <span>Wing Loading (WL):</span>
                              <span className="font-bold text-gray-900">
                                10.06 oz./ft² / 32.5 g/dm²
                              </span>
                            </li>
                            <li className="flex justify-between border-b pb-1">
                            ...
                            <li className="flex justify-between pb-1">
                              <span>Control Throws:</span>
                              <span className="font-bold text-gray-900">
                                Aileron: 20° Expo: 300, Elevator: 20° Expo: 300,
                                Rudder: 20° Expo: 300
                              </span>
                            </li>
                          </>
                          ...
                          <>
                            ...
                            <li className="flex justify-between pb-1">
                              <span>CG Location:</span>
                              <span className="font-bold text-gray-900">
                                60 mm from Leading Edge (at Spar)
                              </span>
                            </li>
                          </>
                        )}
                        ...
                        {isViggen ? (
                          <>
                            <li className="flex items-start gap-2">
                              ...
                              <span>
                                EDF: 1000-1200 Watts. Minimum thrust 1100gms.
                              </span>
                            </li>
                            <li className="flex items-start gap-2">
                              <span className="text-[#dd3333]">•</span>
                            ...
                            <li className="flex items-start gap-2">
                              <span className="text-[#dd3333]">•</span>
                              <span>
                                For Thrust Vectoring Use Metal gear servos.
                              </span>
                            </li>
                          </>
                        ) : isMustang ? (
                  ...
                  <div className="bg-white rounded-2xl border-t-4 border-t-[#d48a37] border border-gray-200 p-6 shadow-sm space-y-6">
                    <div className="flex items-center gap-2 border-b pb-3">
                      ...
                      <h4 className="font-extrabold text-[#dd3333] text-base uppercase tracking-wider">
                    ...
                    {isViggen ? (
                      <>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pt-6">
                          <ImagePlaceholder
                            label="Laser Cut Fliteboard Pro Right Wing (Sheet A)"
                            src="/src/assets/images/viggen_sheet_a.jpg?v=2"
                            onClick={() =>
                              setLightbox({
                                src: "/src/assets/images/viggen_sheet_a.jpg?v=2",
                                label:
                                  "Laser Cut Fliteboard Pro Right Wing (Sheet A)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="Laser Cut Fliteboard Pro Sheet B (Fuselage)"
                            src="/src/assets/images/viggen_sheet_b.jpg?v=2"
                            onClick={() =>
                              setLightbox({
                                src: "/src/assets/images/viggen_sheet_b.jpg?v=2",
                                label:
                                  "Laser Cut Fliteboard Pro Sheet B (Fuselage)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="Laser Cut Fliteboard Pro Sheet C (Fuselage Top / Canopy)"
                            src="/src/assets/images/viggen_sheet_c_1.png?v=2"
                            onClick={() =>
                              setLightbox({
                                src: "/src/assets/images/viggen_sheet_c_1.png?v=2",
                                label:
                                  "Laser Cut Fliteboard Pro Sheet C (Fuselage Top / Canopy)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="Laser Cut Fliteboard Pro Sheet C (Nose Module / Battery Box)"
                            src="/src/assets/images/viggen_sheet_c_2.png?v=2"
                            onClick={() =>
                              setLightbox({
                                src: "/src/assets/images/viggen_sheet_c_2.png?v=2",
                                label:
                                  "Laser Cut Fliteboard Pro Sheet C (Nose Module / Battery Box)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="Laser Cut Fliteboard Pro Sheet D (Motor Mount Backplate)"
                            src="/src/assets/images/viggen_sheet_d.png?v=2"
                            onClick={() =>
                              setLightbox({
                                src: "/src/assets/images/viggen_sheet_d.png?v=2",
                                label:
                                  "Laser Cut Fliteboard Pro Sheet D (Motor Mount Backplate)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="Posterboard Exhaust Tube (Curved Template)"
                            src="/src/assets/images/viggen_exhaust_template.jpg?v=2"
                            onClick={() =>
                              setLightbox({
                                src: "/src/assets/images/viggen_exhaust_template.jpg?v=2",
                                label:
                                  "Posterboard Exhaust Tube (Curved Template)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="Thrust Vectoring Mount & Hardware Kit"
                            src="/src/assets/images/viggen_accessories.jpg?v=2"
                            onClick={() =>
                              setLightbox({
                                src: "/src/assets/images/viggen_accessories.jpg?v=2",
                                label: "Thrust Vectoring Mount & Hardware Kit",
                      ...
                      <>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 pt-6">
                          <ImagePlaceholder
                            label="P-51D Mustang Sheet A (Right Wing / Vertical Fin)"
                            src={`${mustangSheetA}?v=3`}
                            onClick={() =>
                              setLightbox({
                                src: `${mustangSheetA}?v=3`,
                                label:
                                  "P-51D Mustang Sheet A (Right Wing / Vertical Fin)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="P-51D Mustang Sheet B (Left Wing / Horizontal Stab)"
                            src={`${mustangSheetB}?v=3`}
                            onClick={() =>
                              setLightbox({
                                src: `${mustangSheetB}?v=3`,
                                label:
                                  "P-51D Mustang Sheet B (Left Wing / Horizontal Stab)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="P-51D Mustang Sheet C (Fuselage / Turtle Deck)"
                            src={`${mustangSheetC}?v=3`}
                            onClick={() =>
                              setLightbox({
                                src: `${mustangSheetC}?v=3`,
                                label:
                                  "P-51D Mustang Sheet C (Fuselage / Turtle Deck)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="P-51D Mustang Sheet D (Power Pod)"
                            src={`${mustangSheetD}?v=3`}
                            onClick={() =>
                              setLightbox({
                                src: `${mustangSheetD}?v=3`,
                                label: "P-51D Mustang Sheet D (Power Pod)",
                      ...
                      <>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-6">
                          <ImagePlaceholder
                            label="Spitfire Sheet A (Left Wing / Power Pod)"
                            src={spitfireSheetA}
                            onClick={() =>
                              setLightbox({
                                src: spitfireSheetA,
                                label:
                                  "Spitfire Sheet A (Left Wing / Power Pod)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="Spitfire Sheet B (Right Wing / Tail Fin)"
                            src={spitfireSheetB}
                            onClick={() =>
                              setLightbox({
                                src: spitfireSheetB,
                                label:
                                  "Spitfire Sheet B (Right Wing / Tail Fin)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="Spitfire Sheet C (Fuselage)"
                            src={spitfireSheetC}
                            onClick={() =>
                              setLightbox({
                                src: spitfireSheetC,
                          ...
                          />
                          <ImagePlaceholder
                            label="Spitfire Curved Nose & Exhaust Templates"
                            src={spitfireExTemp}
                            onClick={() =>
                              setLightbox({
                                src: spitfireExTemp,
                                label:
                                  "Spitfire Curved Nose & Exhaust Templates",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="Spitfire Hardware Accessories Pack"
                            src={spitfireAcc}
                            onClick={() =>
                              setLightbox({
                                src: spitfireAcc,
                      ...
                      <>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pt-6">
                          <ImagePlaceholder
                            label="FT-Storch Sheet A (Slats / Dihedral Gauge)"
                            src={storchSheetA}
                            onClick={() =>
                              setLightbox({
                                src: storchSheetA,
                                label:
                                  "FT-Storch Sheet A (Slats / Dihedral Gauge)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="FT-Storch Sheet B (Tail Stabilizers / Power Pod)"
                            src={storchSheetB}
                            onClick={() =>
                              setLightbox({
                                src: storchSheetB,
                                label:
                                  "FT-Storch Sheet B (Tail Stabilizers / Power Pod)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="FT-Storch Wing Sheet"
                            src={storchSheetWing}
                            onClick={() =>
                              setLightbox({
                                src: storchSheetWing,
                          ...
                          />
                          <ImagePlaceholder
                            label="FT-Storch Fuselage Sheet A"
                            src={storchSheetFuselage}
                            onClick={() =>
                              setLightbox({
                                src: storchSheetFuselage,
                                label: "FT-Storch Fuselage Sheet A",
                          ...
                          />
                          <ImagePlaceholder
                            label="FT-Storch Fuselage Sheet B"
                            src={storchSheetC}
                            onClick={() =>
                              setLightbox({
                                src: storchSheetC,
                          ...
                          />
                          <ImagePlaceholder
                            label="FT-Storch Accessories & Hardware Pack"
                            src={storchAccessories}
                            onClick={() =>
                              setLightbox({
                                src: storchAccessories,
                                label: "FT-Storch Accessories & Hardware Pack",
                      ...
                      <>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pt-6">
                          <ImagePlaceholder
                            label="FT-Explorer Sheets A & B (Fuselage / Power Pod)"
                            src={explorerSheetAB}
                            onClick={() =>
                              setLightbox({
                                src: explorerSheetAB,
                                label:
                                  "FT-Explorer Sheets A & B (Fuselage / Power Pod)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="FT-Explorer Sheet D (Tail / Stabilizers)"
                            src={explorerSheetD}
                            onClick={() =>
                              setLightbox({
                                src: explorerSheetD,
                                label:
                                  "FT-Explorer Sheet D (Tail / Stabilizers)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="FT-Explorer Wing Spar Sheet"
                            src={explorerSheetWingSpar}
                            onClick={() =>
                              setLightbox({
                                src: explorerSheetWingSpar,
                                label: "FT-Explorer Wing Spar Sheet",
                          ...
                          />
                          <ImagePlaceholder
                            label="FT-Explorer Wing Sheet (Left Panel)"
                            src={explorerSheetWingLeft}
                            onClick={() =>
                              setLightbox({
                                src: explorerSheetWingLeft,
                                label: "FT-Explorer Wing Sheet (Left Panel)",
                          ...
                          />
                          <ImagePlaceholder
                            label="FT-Explorer Wing Sheet (Right Panel)"
                            src={explorerSheetWingRight}
                            onClick={() =>
                              setLightbox({
                                src: explorerSheetWingRight,
                                label: "FT-Explorer Wing Sheet (Right Panel)",
                          ...
                          />
                          <ImagePlaceholder
                            label="FT-Explorer Wing Sheet (Left Panel Alt)"
                            src={explorerSheetWingLeftAlt}
                            onClick={() =>
                              setLightbox({
                                src: explorerSheetWingLeftAlt,
                                label:
                                  "FT-Explorer Wing Sheet (Left Panel Alt)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="FT-Explorer Decals & Stickers"
                            src={explorerDecals}
                            onClick={() =>
                              setLightbox({
                                src: explorerDecals,
                      ...
                      <>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pt-6">
                          <ImagePlaceholder
                            label="FT-Spear Fuselage & Spar Sheet"
                            src={spearSheet1}
                            onClick={() =>
                              setLightbox({
                                src: spearSheet1,
                          ...
                          />
                          <ImagePlaceholder
                            label="FT-Spear Wing Cutouts & Elevons Sheet"
                            src={spearSheet2}
                            onClick={() =>
                              setLightbox({
                                src: spearSheet2,
                          ...
                          />
                          <ImagePlaceholder
                            label="FT-Spear Sheet E (Right Wing Panel)"
                            src={spearSheetE}
                            onClick={() =>
                              setLightbox({
                                src: spearSheetE,
                          ...
                          />
                          <ImagePlaceholder
                            label="FT-Spear Sheet F (Left Wing Panel)"
                            src={spearSheetF}
                            onClick={() =>
                              setLightbox({
                                src: spearSheetF,
                          ...
                          />
                          <ImagePlaceholder
                            label="FT-Spear Accessories & Hardware Pack"
                            src={spearAccessories}
                            onClick={() =>
                              setLightbox({
                                src: spearAccessories,
                                label: "FT-Spear Accessories & Hardware Pack",
                          ...
                          />
                          <ImagePlaceholder
                            label="FT-Spear Elevon & Spar Sheet"
                            src={spearSheet6}
                            onClick={() =>
                              setLightbox({
                                src: spearSheet6,
                      ...
                      <>
                        ...
                        <div className="space-y-6 pt-6">
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <ImagePlaceholder
                              label="FT-22 Raptor Sheets A & B (Fuselage & Canopy)"
                              src={raptorUser1}
                              onClick={() =>
                                setLightbox({
                                  src: raptorUser1,
                                  label:
                                    "FT-22 Raptor Sheets A & B (Fuselage & Canopy)",
                                })
                              }
                            />
                            <ImagePlaceholder
                              label="FT-22 Raptor Wing & Elevon Sheet"
                              src={raptorUser2}
                              onClick={() =>
                                setLightbox({
                                  src: raptorUser2,
                          ...
                          <div className="w-full">
                            <ImagePlaceholder
                              label="FT-22 Raptor Accessories & Hardware Kit"
                              src={raptorUser3}
                              onClick={() =>
                                setLightbox({
                                  src: raptorUser3,
                                  label:
                                    "FT-22 Raptor Accessories & Hardware Kit",
                                })
                              }
                              aspect="aspect-[3/1]"
                            />
                      ...
                      <>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 pt-6">
                          <ImagePlaceholder
                            label="Old Fogey Power Pod Sheet"
                            src={fogeyUser2}
                            onClick={() =>
                              setLightbox({
                                src: fogeyUser2,
                          ...
                          />
                          <ImagePlaceholder
                            label="Old Fogey Fuselage Sheet"
                            src={fogeyUser1}
                            onClick={() =>
                              setLightbox({
                                src: fogeyUser1,
                          ...
                          />
                          <ImagePlaceholder
                            label="Old Fogey Accessories & Hardware Kit"
                            src={fogeyUser4}
                            onClick={() =>
                              setLightbox({
                                src: fogeyUser4,
                      ...
                      <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-2 text-xs sm:text-sm font-medium text-gray-700">
                          <div className="space-y-2">
                            <p>• Fliteboard Pro</p>
                            <p>
                              • Precision Laser-cut Fliteboard Pro sheets (4
                              Nos.)
                            </p>
                            <p>• Aeroply Parts</p>
                            <p>• Motor Mount</p>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pt-6">
                          <ImagePlaceholder
                            label="Bloody Baron Sheets A & B (Fuselage & Doublers)"
                            src={baronUser1}
                            onClick={() =>
                              setLightbox({
                                src: baronUser1,
                                label:
                                  "Bloody Baron Sheets A & B (Fuselage & Doublers)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="Bloody Baron Sheets C & D (Power Pod & Stabilizers)"
                            src={baronUser2}
                            onClick={() =>
                              setLightbox({
                                src: baronUser2,
                                label:
                                  "Bloody Baron Sheets C & D (Power Pod & Stabilizers)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="Bloody Baron Wing Panel Sheet"
                            src={baronUser3}
                            onClick={() =>
                              setLightbox({
                                src: baronUser3,
                      ...
                      <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-2 text-xs sm:text-sm font-medium text-gray-700">
                          <div className="space-y-2">
                            ...
                            <p>
                              • EPP Printed Horizontal & Vertical Stabilizer
                            </p>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-6">
                          <ImagePlaceholder
                            label="Simple Trainer Left Wing"
                            src="/src/assets/images/trainer_left_wing.png?v=2"
                            onClick={() =>
                              setLightbox({
                                src: "/src/assets/images/trainer_left_wing.png?v=2",
                                label: "Simple Trainer Left Wing",
                          ...
                          />
                          <ImagePlaceholder
                            label="Simple Trainer Right Wing"
                            src="/src/assets/images/trainer_right_wing.png?v=2"
                            onClick={() =>
                              setLightbox({
                                src: "/src/assets/images/trainer_right_wing.png?v=2",
                                label: "Simple Trainer Right Wing",
                          ...
                          />
                          <ImagePlaceholder
                            label="Simple Trainer Sheets A & B (Fuselage / Stabilizers)"
                            src="/src/assets/images/trainer_sheets.png?v=2"
                            onClick={() =>
                              setLightbox({
                                src: "/src/assets/images/trainer_sheets.png?v=2",
                                label:
                                  "Simple Trainer Sheets A & B (Fuselage / Stabilizers)",
                              })
                            }
                          />
                          <ImagePlaceholder
                            label="EPP Trainer Hardware & Accessories Kit"
                            src="/src/assets/images/trainer_accessories.jpg?v=2"
                            onClick={() =>
                              setLightbox({
                                src: "/src/assets/images/trainer_accessories.jpg?v=2",
                                label: "EPP Trainer Hardware & Accessories Kit",
                  ...
                  <div className="bg-[#fcfcfc] rounded-2xl border border-gray-200 p-6 shadow-xs grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
                    <div className="md:col-span-7 space-y-4">
                      <h4 className="font-extrabold text-[#dd3333] text-base uppercase tracking-wider border-b pb-2">
                        Kit Options - Electronics
                      </h4>
                      ...
                      <ul className="space-y-2 text-xs sm:text-sm font-medium text-gray-700 pl-4 list-disc">
                        {isViggen ? (
                          <>
                            ...
                            <li>
                              Thrust Vectoring Metal Wires & Pushrod Connectors
                            </li>
                          ...
                          <>
                            ...
                            <li>
                              2 Nos. 30Amp Brushless ESCs (Speed controllers)
                            </li>
                          ...
                          <>
                            ...
                            <li>
                              3 Nos. Male Bullet Connectors ( For Motor Leads )
                            </li>
                    ...
                    <div className="md:col-span-5 rounded-xl overflow-hidden border border-gray-200 shadow-sm aspect-4/3 bg-gray-50 flex items-center justify-center p-2">
                      <img
                        src={
                          isViggen
                            ? "/src/assets/images/viggen_electronics.jpg"
                            : isMustang
                              ? mustangElectronics
                              : isSpitfire
                                ? spitfireElectronics
                                : isGuineaPig
                                  ? guineaPigElectronics
                                  : isStorch
                                    ? storchElectronics
                                    : isExplorer
                                      ? explorerElectronics
                                      : isSpear
                                        ? "/src/assets/images/spear_electronics_user.jpg"
                                        : isRaptor
                                          ? "/src/assets/images/raptor_electronics_user.jpg"
                                          : isOldFogey
                                            ? "/src/assets/images/fogey_electronics_user.jpg"
                                            : isBaron
                                              ? "/src/assets/images/baron_electronics_user.jpg"
                                              : "/src/assets/images/epp_trainer_electronics.jpg?v=2"
                        }
                        alt="Optional electronics package"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
                    ...
                    <div ref={progressRef} className="md:col-span-7 space-y-5">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-xs sm:text-sm font-bold text-gray-800">
                          <span>
                            {isViggen
                              ? "Performance - HIGH SPEED JET"
                              : isSpitfire
                                ? "Performance - SCALE FIGHTER"
                                : isMustang
                                  ? "Performance - SPORT WARBIRD"
                                  : "Cost Effective - ULTRA ECONOMICAL"}
                          </span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-3.5 overflow-hidden">
                          <div
                    ...
                    <div className="md:col-span-5 bg-red-50/50 border border-red-100 rounded-2xl p-5 shadow-xs flex gap-3.5">
                      ...
                      <div className="space-y-2">
                        <h4 className="font-extrabold text-[#dd3333] text-sm uppercase tracking-wider">
                          Expert Advice
                        </h4>
                  ...
                  <div className="pt-8 border-t border-gray-200 space-y-6">
                    ...
                    <p className="text-gray-700 text-xs sm:text-sm font-medium leading-relaxed">
                      All our kits are securely packaged to ensure they reach
                      you safely. Lasercut Sheets are first shrink wrapped,
                      small parts are sealed in plastic bags, and then the
                      entire kit is bubble wrapped, before being put in a custom
                      made 5-ply corrugated Box.
                    </p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-2">
                      <div className="space-y-2 text-center">
                        <div className="rounded-xl overflow-hidden border border-gray-200 aspect-video shadow-xs bg-gray-50">
                          <img
                            src="/src/assets/images/packaging_shrink_wrapped.png"
                            alt="Shrink Wrapped"
                            className="w-full h-full object-cover"
                          />
                        </div>
                      ...
                      <div className="space-y-2 text-center">
                        <div className="rounded-xl overflow-hidden border border-gray-200 aspect-video shadow-xs bg-gray-50">
                          <img
                            src="/src/assets/images/packaging_bubble_wrapped.png"
                            alt="Bubble Wrapped"
                            className="w-full h-full object-cover"
                          />
                        </div>
                        <span className="text-xs text-gray-500 font-semibold block">
                          Bubble Wrapped for Further Protection
                        </span>
                      </div>
                      ...
                      <div className="space-y-2 text-center">
                        <div className="rounded-xl overflow-hidden border border-gray-200 aspect-video shadow-xs bg-gray-50">
                          <img
                            src="/src/assets/images/packaging_corrugated_box.png"
                            alt="Corrugated box"
                            className="w-full h-full object-cover"
                          />
                        </div>
                        <span className="text-xs text-gray-500 font-semibold block">
                          Shipped in Strong Corrugated box
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-6 pt-4 items-center justify-between border-t border-gray-100">
                      <div className="space-y-3 flex-1">
                        <p className="text-gray-700 text-xs sm:text-sm font-medium leading-relaxed">
                          All Kits are shipped via India Post. This is because,
                          Private couriers charge based on Volumetric Pricing,
                          which is often very expensive. We stand by our kits
                          and guarantee that in case of any major damages during
                          transit, or any manufacturing defects , we shall
                          gladly help arrange replacements.
                        </p>
                        <p className="text-gray-700 text-xs sm:text-sm font-medium leading-relaxed">
                          IndiaPost Service is a Trackable service, and we
                          provide tracking details, usually within 1 working
                          day.
                        </p>
                      </div>
                      ...
                      <div className="flex flex-col items-center border border-gray-200 rounded-xl p-3 bg-white w-40 flex-shrink-0">
                        ...
                        <div className="w-24 h-12 bg-[#D12B2B] relative rounded-md overflow-hidden my-1 flex items-center justify-center">
                          <svg
                            className="w-16 h-10 text-[#FFCC00]"
                            fill="currentColor"
                            viewBox="0 0 100 50">
                        ...
                        <div className="text-xs font-extrabold text-[#D12B2B] tracking-wider leading-none">
                          India Post
                        </div>
                      </div>
                    </div>
              ...
              {activeTab === "shipping" && (
                <div className="space-y-3">
                  ...
                  <p>
                    We pack every aircraft inside dual-wall reinforced
                    high-density corrugated flight boxes to ensure your
                    VT-Simple Trainer arrives in pristine condition.
                  </p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                    <div className="p-4 bg-gray-50 rounded-2xl">
                      ...
                      <p className="text-xs text-gray-500">
                        3-5 Business Days (₹1,200 or FREE on orders &gt;
                        ₹12,000)
                      </p>
                    </div>
                    <div className="p-4 bg-gray-50 rounded-2xl">
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    {productReviews.map((rev) => (
                      <div
                        key={rev.id}
                        className="p-4 bg-gray-50 rounded-2xl space-y-2 border border-gray-100">
                        <div className="flex items-center justify-between">
          ...
          <div className="border-t border-gray-200 pt-10">
            <div className="text-center max-w-3xl mx-auto mb-8 space-y-2">
              ...
              <p className="text-xs sm:text-sm text-gray-500">
                Follow our master builder's video guide to assemble the{" "}
                {product.name} airframe step-by-step in under 10 minutes.
              </p>
            </div>
              ...
              <div className="lg:col-span-5 flex flex-col justify-between space-y-6">
                <div>
                  ...
                  <div className="space-y-3.5">
                    <div className="flex items-start gap-3 p-3 rounded-xl hover:bg-slate-50 transition cursor-pointer">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-gray-500">
                          Checking laser-cut EPP sheets and structural hardware
                          (0:00 - 2:15)
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3 p-3 rounded-xl hover:bg-slate-50 transition cursor-pointer">
                      ...
                      <div>
                        <h4 className="font-semibold text-xs text-gray-900">
                          Fuselage & Wing Joinery
                        </h4>
                        <p className="text-[11px] text-gray-500">
                          Inserting carbon spars and locking the main wing
                          panels (2:15 - 5:45)
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3 p-3 rounded-xl hover:bg-slate-50 transition cursor-pointer">
                      ...
                      <div>
                        <h4 className="font-semibold text-xs text-gray-900">
                          Servos & Pushrods Linkage
                        </h4>
                        <p className="text-[11px] text-gray-500">
                          Routing servo extensions and connecting control horn
                          pushrods (5:45 - 8:30)
                        </p>
                      </div>
                    </div>
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 flex items-center gap-3">
                  ...
                  <p className="text-[11px] text-gray-600 leading-relaxed">
                    <strong>Need additional help?</strong> Download the full PDF
                    instruction guide from the support page or connect with our
                    pilot hotline.
                  </p>
                </div>
              </div>
