...
export function useSEO({ title, description, image, canonical, jsonLd }) {
  useEffect(() => {
    const siteTitle = 'Rc Plane Hub | Learn to Fly with Confidence';
    const finalTitle = title ? `${title} | Rc Plane Hub` : siteTitle;
    document.title = finalTitle;

    const metaDescription = description || 'Rc Plane Hub - Home of the VT-Simple Trainer. Premium RC Aircraft designed for beginner & enthusiast pilots. Built with VT-Stabilize 6-axis gyro technology.';
