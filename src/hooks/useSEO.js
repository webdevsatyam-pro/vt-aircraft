...
export function useSEO({ title, description, image, canonical, jsonLd }) {
  useEffect(() => {
    const siteTitle = 'VT Aircraft | Learn to Fly with Confidence';
    const finalTitle = title ? `${title} | VT Aircraft` : siteTitle;
    document.title = finalTitle;

    const metaDescription = description || 'VT Aircraft - Home of the VT-Simple Trainer. Premium RC Aircraft designed for beginner & enthusiast pilots. Built with VT-Stabilize 6-axis gyro technology.';
    const metaImage = image || 'https://vtaircraft.com/images/vt_trainer_hero.jpg';
